    ...
    def __init__(self, params, energy, config, name='IPALM'):
        ...
        with tf.variable_scope(name):
            for param_idx in range(len(params.get(as_dict=False))):
                ...
                self._grad_var.append(tf.placeholder(current_param.dtype, shape=current_param.get_shape(),
                                               name='grad_var_' + current_param.name.split(':')[0]))
                self._var_old.append(tf.placeholder(current_param.dtype, shape=current_param.get_shape(),
                ...
                self._update_var.append(tf.assign(current_param, self._var_old[param_idx]))
